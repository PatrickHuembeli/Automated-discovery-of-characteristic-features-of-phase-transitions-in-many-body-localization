...
class DANNBuilder(object):
    ...
    def _build_feature_extractor(self, model_input):
        ...
        net = Convolution1D(
            nb_filters, nb_conv, border_mode='valid',
    ...
    def _build_classifier(self, model_input):
        ...
        net = Dropout(0.5)(net)
        net = Dense(
            nb_classes, activation='softmax', name='classifier_output')(net)
        return net
    ...
    def build_dann_model(self, main_input, plot_model=False):
        ...
        net = Lambda(
            lambda x: K.switch(K.learning_phase(), x[:int(batch_size // 2), :], x),
            output_shape=
            lambda x: ((batch_size // 2, ) + x[1:]) if _TRAIN else x[0:])(net)
        net = self._build_classifier(net)
...
unsuperivised_weight_path = folder + 'unsupervised_MBL_' + str(
    len_x_train) + '.npy'
...
src_model = builder.build_source_model(
    main_input)  # this is CNN and feature extractor
...
if train_dann:
    ...
    for i in range(nb_epoch):
        ...
        if i % 10 == 0:
            np.save(folder + 'weights' + str(i),
                    dann_model.get_weights())  # save weights after each epoch
        device = "gpu"
        with tf.device("/" + device + ":0"):
            for (xb, yb) in src_gen:
                ...
                lr = 0.0005 / (
                    1. + 10 * p
                )**0.75  #0.00005 / (1. + 10 * p)**0.75 # # 0.01 / (1. + 10 * p)**0.75 #
...
if evaluate_model:
    ...
    for k in range(len(epsilon_list)):
        ...
        for l in range(len(hmax_list)):  #100_samp_EVALUATE_N12eps0.9hmax0.9
            ...
            val = 0.0
            hmax = hmax_list[l]
            filename = 'EVALUATION_FILES/' + '00EVALUATE_N' + str(
...
if analyze_predictions:
    ...
    for i in range(20):  #because we have 20 disorder realizations
        avg_list.append(
            np.sum(target_phase[i * average:(i + 1) * average]) / average)
    plt.clf()
