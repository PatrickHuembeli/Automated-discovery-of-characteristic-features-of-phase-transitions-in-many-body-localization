...
def calc_samples(epsilon, hmax, margin , stat):
    ...
    energies, states = H.eigsh(k=2*margin+2, sigma=e, maxiter=1E4,return_eigenvectors= True)
    states = states.T
    ...
    for epsilon in epsilon_list:
        ...
        for hmax in hmax_list:
            ...
            for i in range(samples):
                ...
                stat = calc_samples(epsilon, hmax, margin, stat)
            liste.append(stat/samples)
        np.save(statfolder + 'SPARSE_SOURCE_statistics_N_'+str(L)+'epsilon_'+str(epsilon), liste)
    save_H5(filename)
    ...
    for epsilon in epsilon_list:
        ...
        for hmax in hmax_list:
            ...
            for i in range(samples):
                ...
                stat = calc_samples(epsilon, hmax, margin, stat)
            liste.append(stat/samples)
        np.save(statfolder + 'SPARSE_TARGET_statistics_N_'+str(L)+'epsilon_'+str(epsilon), liste)
    save_H5(filename)
    ...
    if evaluation:
        ...
        for epsilon in epsilon_list:
            ...
            for hmax in hmax_list:
                ...
                labels_fullspec = []
                for i in range(samples):
                    print('SPARSE_EVALUATE, ', 'epsilon:', epsilon, 'hmax:', hmax, i)
                    stat = calc_samples(epsilon, hmax, margin, stat)
                liste.append(stat/samples)
                filename = folder + str(samples)+'_samp_EVALUATE_N' + str(L) + 'eps'+ str(epsilon) + 'hmax' + str(hmax) +'.h5'
                print(len(state_list))
                save_H5(filename)
            np.save(statfolder + 'SPARSE_EVALUATION_statistics_N_'+str(L)+'_sampels_'+str(samples)+'epsilon_'+str(epsilon), liste)
