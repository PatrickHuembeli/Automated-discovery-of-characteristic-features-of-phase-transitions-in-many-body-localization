...
def calc_samples(epsilon, hmax, margin, stat):
    ...
    stat += mean
    for i in states[max(1, epsilon_index - margin
                        ):min(len(energies) - 1, epsilon_index + margin)]:
        psi = np.asarray(i)
...
if source:
    ...
    for epsilon in epsilon_list:
        ...
        np.save('SOURCE_statistics_N_' + str(L) + 'epsilon_' + str(epsilon),
                liste)
    save_H5(filename)
    ...
    for epsilon in epsilon_list:
        ...
        for hmax in hmax_list:
            ...
            for i in range(samples):
                ...
                stat = calc_samples(epsilon, hmax, margin, stat)
            liste.append(stat / samples)
        np.save('TARGET_statistics_N_' + str(L) + 'epsilon_' + str(epsilon),
                liste)
    save_H5(filename)
...
if evaluation:
    ...
    for epsilon in epsilon_list:
        ...
        for hmax in hmax_list:
            ...
            #save_H5(filename)
        np.save(
            'EVALUATION_statistics_N_' + str(L) + '_sampels_' + str(samples) +
            'epsilon_' + str(epsilon), liste)
